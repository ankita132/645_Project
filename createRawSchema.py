...
def createRawSchemaTable():
    ...
    cursor.execute("COPY Adult FROM './adult.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER MATCH, FORCE_NULL(age, fnlwgt, education_num, capital_loss, capital_gain, hours_per_week))")
    cursor.execute("COPY Adult FROM './adult_test.csv' WITH (FORMAT CSV, DELIMITER ',', HEADER MATCH, FORCE_NULL(age, fnlwgt, education_num, capital_loss, capital_gain, hours_per_week))")
